...
class CommonSpider(scrapy.Spider):
    ...
    def __init__(self, section):
        ...
        path_config = os.path.join(self.base_source_path, 'config', 'config.cfg')
        config.read(path_config)

        # 'TARGET_INFO'
        # section = section and 'TARGET_INFO' or section
        section = section
        self.parsing_url = str(config.get(section, 'parsing_url')).replace('__page__', '%d')
    ...
    def parse_clien(self, response):
        ...
        path_page_store = os.path.join(self.base_source_path, 'page.txt')
        path_max_seq_store = os.path.join(self.base_source_path, 'max_article.txt')

        '''
        print("------"*10)
        print(path_max_seq_store)
        print(path_page_store)
        print("------"*10)
        '''

        # Books 와 Resources 란 이름의 파일 생성
        with open(path_page_store, 'wb') as f:
            # url의 내용 통째로 저장
